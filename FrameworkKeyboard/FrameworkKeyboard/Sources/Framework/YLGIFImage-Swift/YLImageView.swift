...
class YLImageView : UIImageView {
    ...
    private lazy var displayLink:CADisplayLink = CADisplayLink(target: self, selector: #selector(YLImageView.changeKeyFrame(dpLink:)))
    ...
    override var image: UIImage! {
        ...
        set{
            ...
            if newValue is YLGIFImage {
                ...
                if let Img = self.animatedImage!.getFrame(index: 0) {
                    super.image = Img
        ...
        if let animatedImg = self.animatedImage {
            if self.currentFrameIndex < animatedImg.frameImages.count {
                ...
                while self.accumulator >= frameDura.doubleValue
                {
                    ...
                    if let Img = animatedImg.getFrame(index: UInt(self.currentFrameIndex)) {
